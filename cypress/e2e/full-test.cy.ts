...
describe('Home View - Events Management', () => {
  ...
  it('should open and close the create event modal', () => {
    cy.get('#events-add-new-event').click({ force: true })

    cy.contains('New Event').should('exist') // Modal header
    cy.get('#modal-cancel-button').should('exist')
    cy.get('#modal-cancel-button').click({ force: true })
  ...
  it('should create a new event', () => {
    ...
    cy.get('#events-add-new-event').click({ force: true })
    cy.get('#identification').type(new Date().toISOString())
    ...
    cy.get('#modal-save-button').click({ force: true })
    cy.wait('@createEvent')
  ...
  it('should select and open an existing event', () => {
    ...
    cy.get('tbody tr').last().click({ force: true })
  ...
  it('should delete an event with confirmation', () => {
    ...
    cy.get('tbody tr').last().click({ force: true })
    cy.get('#modal-delete-button').click({ force: true })
